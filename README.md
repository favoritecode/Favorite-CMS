...
Copy-ready instructions for generating a new declarative Plugin, isolated fixed Worker Tool, or presentation-only Theme without redesigning Core are in [Generating Plugins, Themes, and Tools](docs/generating-plugins-themes-and-tools.md).
...
Authorized Content operators can migrate Blogger posts/pages from the Admin **Import Blogger** action using Blogger's XML backup. Imports are bounded, sanitized, never overwrite existing Content, and default to private drafts; preserving Blogger publication state additionally requires explicit publish permission. Article/code bodies support up to 2,000,000 sanitized characters. Media remains separately bounded to 10 MB per supported, signature-validated file for Storage safety.
