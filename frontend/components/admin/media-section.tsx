...
export function MediaSection() {
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await adminRequest<MediaItem>("/admin/manage/transport/media", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ file_name: form.get("name"), mime_type: "text/plain", text: form.get("text") }) });
  ...
  return <div className="space-y-5">
    ...
    {uploadOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4" role="presentation"><section role="dialog" aria-modal="true" aria-labelledby="upload-title" className="w-full max-w-xl rounded-md bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><p className="text-xs font-semibold uppercase text-sky-700">Media library</p><h2 id="upload-title" className="font-semibold">Add text document</h2></div><button type="button" title="Close" aria-label="Close upload" onClick={() => !submitting && setUploadOpen(false)} className="rounded-md p-2 text-slate-500 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-600"><X className="size-5" /></button></div><form className="grid gap-5 p-5" onSubmit={upload}><label className="grid gap-2 text-sm font-medium">File name<input name="name" className={fieldClass} required maxLength={255} pattern="[A-Za-z0-9][A-Za-z0-9._ -]*" placeholder="editorial-notes.txt" /><span className="text-xs font-normal text-slate-500">A safe file name without folders or paths.</span></label><label className="grid gap-2 text-sm font-medium">Document content<textarea name="text" className={`${fieldClass} min-h-56 resize-y font-mono leading-6`} required maxLength={10000} placeholder="Plain UTF-8 text" /><span className="text-xs font-normal text-slate-500">Maximum 10,000 UTF-8 bytes.</span></label><div className="flex justify-end gap-2"><button type="button" className={secondaryButton} onClick={() => setUploadOpen(false)} disabled={submitting}>Cancel</button><button type="submit" className={primaryButton} disabled={submitting}><Upload className="size-4" />{submitting ? "Adding document" : "Add document"}</button></div></form></section></div>}
  </div>;
}
